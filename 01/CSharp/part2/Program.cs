...
namespace CSharp
{
    class Program {
        ...
        static void Main(string[] args) {
            ...
            for (int i = calorieList.Count - 3; i < calorieList.Count; i++)
            {
                if (topThreeTotalCalories == 0)
                {
                    topThreeTotalCalories = calorieList[i];
                }
                else
                {
                    topThreeTotalCalories += calorieList[i];
            ...
            WriteLine($"The top three elves are carrying {topThreeTotalCalories} calories");
        }
    }
}
